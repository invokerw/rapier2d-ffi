...
**BVH is only refreshed during `step`:** Newly inserted colliders — and colliders moved via `rp_collider_set_position / set_rotation / set_pose` (which only updates the parent body) — do NOT appear in the broad-phase BVH until the next `rp_world_step` runs `take_modified` → `broad_phase.update(...)`. Any spatial query between the mutation and the next step will miss them. `rp_world_update_query_pipeline` addresses this without advancing simulation: it syncs collider poses from their parent bodies and then calls `broad_phase.set_aabb(params, handle, aabb)` for every enabled collider. `set_aabb` is used instead of `broad_phase.update` on purpose — `update` would consume `ColliderSet::take_modified()` and drain pair events, causing the subsequent `step` to miss collision callbacks for those colliders. `set_aabb` leaves the modified list intact.
